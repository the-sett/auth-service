...
type_instance(account, entity_type, [fields([unique(key, fields([property(uuid, string, "uuid", true)])), unique(not_key, fields([property(username, username, "username", false)])), property(password, string, "password", false), property(salt, string, "salt", false), property(root, boolean, "root", false), collection(set, roles, no_parent, fields([component_ref(role, role, false, _, _, false)]))]), views([])]).

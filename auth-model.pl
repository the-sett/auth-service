...
type_instance(account, entity_type, [fields([unique(key, fields([property(username, username, "username")])), property(password, string, "password"), property(root, boolean, "root"), collection(set, roles, no_parent, fields([component_ref(role, role, false, _, _)]))]), views([])]).
